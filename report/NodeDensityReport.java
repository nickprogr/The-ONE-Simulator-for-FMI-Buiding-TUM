...
extends SamplingReport {
	...
	protected void sample(final List<DTNHost> hosts) {
		...
		for (final DTNHost host : hosts) {
			...
			if(!(location.getX() == 200.0d && location.getY() == 200.0d)) {
				final int yBucket = (int) (location.getX() / this.divisionWidth);
				final int xBucket = (int) (location.getY() / this.divisionHeight);
	...
	private void outputGnuplotPrefix() {
		...
		super.write("set yrange [" + (this.verticalCount - 1) + ".5:-0.5]");
